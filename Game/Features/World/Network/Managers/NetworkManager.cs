...
namespace Jogo25D.Network
{
    ...
    public partial class NetworkManager : Node
    {
		...
		public async void FinishPeerJoin(long id, CharacterSaveData character)
		{
			...
			{
				await chunkStreamingManager.PreloadSpawnAreaAsync(ChunkStreamingConstants.UPSIDEDOWN_ID, Dimensions.ResolveParent(ChunkStreamingConstants.UPSIDEDOWN_ID), player.Position);

				player.Position = Dimensions.FindGroundSpawnPosition(ChunkStreamingConstants.UPSIDEDOWN_ID, player.Position.X);
			...
			Dimensions.SpawnPlayer(player);

			Dimensions.SpawnPlayerRequest(player);
			...
			var npc = Dimensions.ResolveParent(ChunkStreamingConstants.UPSIDEDOWN_ID)?.GetNodeOrNull<Player>("NPC_Dummy");
			...
			var worldItems = Dimensions.Parents.SelectMany(parent => parent.GetChildren().OfType<WorldItem>());
