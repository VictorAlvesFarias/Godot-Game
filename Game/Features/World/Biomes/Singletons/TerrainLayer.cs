...
namespace Jogo25D.Biomes
{
    ...
    public partial class TerrainLayer : TileMapLayer
    {
        ...
        // Pinta a variante BorderCap (nessa mesma camada, com o PROPRIO terrain_set dela -
        // "ownTerrainSet") nas celulas de "groundCells" que encostam (8 direcoes) em algum tile
        // de OUTRO terrain_set em "groundLayer" - so conta como fronteira de verdade se
        // "groundLayer" tambem for um TerrainLayer e os dois terrain_set NAO estiverem marcados
        // como conectados nele (senao desenharia uma borda decorativa em cima de uma costura que
        // o chao ja deixa lisa).
        public void PaintBorderCap(TileMapLayer groundLayer, IReadOnlyCollection<Vector2I> groundCells, int groundTerrainSet, int ownTerrainSet)
        {
            if (!_interiorTileByTerrainSet.TryGetValue(ownTerrainSet, out var ownInteriorTile))
            {
                return;
            }
            ...
            foreach (var cell in groundCells)
            {
                ...
                if (TouchesForeignTerrain(groundLayer, cell, groundTerrainSet))
                {
            ...
            // O calculo de conexao so enxerga o que ja esta pintado NESSA camada de overlay -
            // como so as celulas da linha de juncao sao pintadas ali, o lado que vira pro
            // INTERIOR do proprio terreno pareceria "vazio", mesmo o chao la embaixo sendo
            // solido do mesmo terreno. Disfarca temporariamente esses vizinhos interiores (fora
            // da linha de juncao) de tile BorderCap interior - mesmo truque do Connect(), so que
            // aqui e pro proprio lado. Depois do calculo, os vizinhos disfarcados voltam a ficar
            // vazios (o overlay so mostra a linha de fato).
            ...
            foreach (var cell in junctionCells)
            {
                foreach (var offset in NeighborOffsets)
                {
                    ...
                    if (neighborTileData == null || neighborTileData.TerrainSet != groundTerrainSet)
                    {
                        disguisedCells.Remove(neighbor);
                    ...
                    SetCell(neighbor, ownInteriorTile.SourceId, ownInteriorTile.AtlasCoord);
                }
            }
            ...
            SetCellsTerrainConnect(junctionCellsArray, ownTerrainSet, 0, false);
        ...
        private static bool TouchesForeignTerrain(TileMapLayer groundLayer, Vector2I cell, int terrainSet)
        {
            var groundTerrainLayer = groundLayer as TerrainLayer;
            ...
            foreach (var offset in NeighborOffsets)
            {
                ...
                if (groundTerrainLayer != null && groundTerrainLayer.AreConnected(terrainSet, neighborTileData.TerrainSet))
                {
                    continue;
                }
